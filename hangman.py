...
class HumanPlayer(Player):
    """Subclass of the Player class for the human player. Inherits Player.

    Attributes:
        name (str): Inherited name of the Player.
    """
    
    def turn(self, GameState):
        """Prints the game board and the prompts the player to guess a 
            letter or solve the entire puzzle.

        Args:
            GameState (obj): Object representing the current state of
                the game.

        Returns:
            str: Value typed by the player.
        ...
        print(GameState)
        self.name_input = input(f'{self.name}, guess a letter or type a word'
                                ' to solve the puzzle: ')
        return self.name_input

class ComputerPlayer(Player):
    """Subclass of the Player class for the computer player. Inherits Player.

    Attributes:
        name (str): Inherited name of the Player.
        vocab_list (list): List of strings that computer can draw from when 
            making turns.
    """
    ...
    def turn(self, GameState):
        """Takes a computer player's turn.
        ...
        Returns:
            str: Value of the computer's turn.
        """
        comp_word = choice(list(GameState.unguessed)).upper()
        if GameState.match(comp_word) == False:
            GameState.bad_guesses.add(comp_word)
        else:
            GameState.good_guesses.add(comp_word)            
        return comp_word()
